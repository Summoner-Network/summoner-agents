...
A guarded GPT-powered agent that decides whether to call a **GitHub commits tool exposed via an MCP server** and, when appropriate, returns a structured summary of recent commits for a repository. It composes a prompt from a **personality** and a **format directive**, then uses the GPT output as parameters for an MCP tool call (`github_latest_commits_summary`). If no call is needed, it returns a small diagnostic payload instead.
...
* use GPT to decide whether to call an external tool,
* call an MCP tool over Streamable HTTP and return normalized results.
...
> [!NOTE]
> The overall structure is inspired by [`EchoAgent_2`](../agent_EchoAgent_2/) and built from its GPT-based adaptation [`GPTRespondAgent`](../agent_GPTRespondAgent/). Compared to `GPTGitHubAgent`, this version moves the GitHub logic into an MCP server and replaces direct `aiohttp` calls with an MCP tool call.
...
> The MCP server calls `load_dotenv()` and reads `GITHUB_TOKEN` from the environment and includes it as a `token` header for GitHub API calls. This raises the rate limit and allows access to private repositories (depending on scopes). A minimal setup is:
>
> ```env
> GITHUB_TOKEN=ghp_yourGeneratedTokenHere
> ```
>
> Ensure your `.env` is in `.gitignore`. When `GITHUB_TOKEN` is not set, the MCP server still works, but with lower unauthenticated rate limits and only on public repos.

> [!IMPORTANT]
> **MCP server required.** This agent expects an MCP server exposing the tool:
>
> * `github_latest_commits_summary(owner: str, repo: str, max_commits: int = 5) -> dict`
>
...
7. The GPT output is expected to be either:
   ...
   * an **empty object** `{}` meaning *do not call the tool*, or
   ...
   * if yes, calls the MCP tool:
     ...
     mcp_result = await mcp_call_tool(
         ...
         tool_name="github_latest_commits_summary",
         arguments={
...
Second, start the MCP server for this agent (GitHub tool provider):

```bash
python agents/agent_MCPGitHubAgent/mcp_server.py
```
...
Prepare `gpt_config.json` and `id.json` in `agents/agent_MCPGitHubAgent/`.

A **typical `gpt_config.json` for MCPGitHubAgent** (only the important parts):
...
```json
// agents/agent_MCPGitHubAgent/id.json
...
```bash
python agents/agent_MCPGitHubAgent/agent.py
```
...
These scenarios show how `MCPGitHubAgent` consumes input from `InputAgent` and either:

* calls the MCP GitHub tool when a commit lookup is appropriate, or
...
# Terminal 3: MCP server (GitHub tool)
python agents/agent_MCPGitHubAgent/mcp_server.py

# Terminal 4: MCPGitHubAgent
python agents/agent_MCPGitHubAgent/agent.py
```
...
3. Call the MCP tool `github_latest_commits_summary` with those arguments.
...
```text
> {"instruction":"Do not call GitHub. Just tell me how to structure a commit message."}
```
...
* The agent returns the diagnostic payload (same as Scenario C).
