...
* call the **Wikipedia REST API** via `aiohttp` and return normalized results.
...
> [!NOTE]
> The overall structure is inspired by [`EchoAgent_2`](../agent_EchoAgent_2/) and built from its GPT based adaptation [`GPTRespondAgent`](../agent_GPTRespondAgent/) by changing its prompt and adding an API call layer to the Wikipedia REST API to fulfill Wikipedia search and summary requests.
...
> [!NOTE]
> **No Wikipedia API key is required.** `GPTWikipediaAgent` uses public Wikipedia REST endpoints (title search and page summary). The agent sets a proper `User-Agent` header internally when it creates the `aiohttp.ClientSession`, so you do not need to configure anything else for Wikipedia access.
...
8. The agent sends back a normalized response of the form:
   ...
   If the helper detects missing parameters or no suitable pages, `result` contains an error payload, for example:
...
| `@agent.send(route=...)`             | Builds the GPT prompt, interprets output as tool args, conditionally calls Wikipedia, returns result |
| `agent.logger`                       | Logs hook activity, buffering, Wikipedia calls, and send lifecycle events                            |
...
Prepare `gpt_config.json` and `id.json` in `agents/agent_GPTWikipediaAgent/`.

A typical `gpt_config.json` for `GPTWikipediaAgent` (only the important parts) is:
...
```json
// agents/agent_GPTWikipediaAgent/id.json
...
```bash
python agents/agent_GPTWikipediaAgent/agent.py
```
...
These scenarios show how `GPTWikipediaAgent` consumes input from `InputAgent` and either:

* calls Wikipedia when a lookup is appropriate, or
...
3. Call the Wikipedia title search endpoint with those arguments.
...
`GPTWikipediaAgent` should interpret this as a direct summary request and GPT should output tool args similar to:

```json
{"action": "summary", "title": "Category theory"}
```

The agent calls the page summary endpoint and returns:
...
`GPTWikipediaAgent` should interpret this as a search then summary request and GPT should output tool args like:
...
* that Wikipedia is called when the intent is clearly a Wikipedia style lookup (search, summary, or search then summary), and
* that no call is made when the request is purely explanatory and does not require Wikipedia.
