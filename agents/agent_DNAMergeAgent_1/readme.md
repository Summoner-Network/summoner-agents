...
> [!NOTE]
> This example is the same functional merge as [`DNAMergeAgent_0`](../agent_DNAMergeAgent_0/), but it highlights one specific merge detail: **explicit client binding name rebinding** via `var_name`.

In practice, handler source code often references the client through a module-global name (sometimes `agent`, sometimes `client`). When merging imported clients, the merger must rebind that name to the merged client so calls like `await client.travel_to(...)` keep working after replay.
...
* **Explicit var name rebinding** (`{"var_name": "client", "client": agent_1}`) tells the merger which global name to rebind inside imported handlers.
...
3. The merged agent constructs the merger, but this time it wraps each imported client in a dict:
   ...
   The important specialization vs `DNAMergeAgent_0` is `var_name="client"`.

   What it means:

   * In the imported modules (`agent_p1.py`, `agent_p2.py`), handler functions may reference the client through a module-global called `client`.
   * During merge, `ClientMerger` mutates the handler globals so that **that exact name** (`client`) now points to the merged client instance.
   * This makes client-referencing statements inside handler code continue to work after replay (for example `await client.travel_to(...)`, `await client.quit()`, etc.).

   `rebind_globals={"viz": viz}` is still required for the same reason as before: handlers were written against a module-global `viz`.
...
6. At startup, the visualizer graph is built from the merged client DNA:
   ...
   The script also dumps a richer DNA file (including context) into [`dna.json`](./dna.json):
...
7. Runtime command behavior is unchanged from `DNAMergeAgent_0`:
...
| `{"var_name": "...", "client": c}` | Explicitly tells the merger which module-global name should be rebound to the merged client |
...
```bash
python agents/agent_DNAMergeAgent_1/agent.py
```
...
```bash
python agents/agent_DNAMergeAgent_1/agent.py --config configs/client_config.json
```
