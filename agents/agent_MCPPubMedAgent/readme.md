...
A guarded GPT powered agent that decides whether to call a **PubMed / NCBI E-utilities tool exposed via an MCP server** and, when appropriate, returns a structured response describing one or several scientific articles. It composes a prompt from a **personality** and a **format directive**, then uses the GPT output as parameters for an MCP tool call to `pubmed_handle_request`. If no PubMed call is needed, it returns a small diagnostic payload instead.
...
* use GPT to decide whether to call an external tool and with which ranking (`latest` vs `most relevant`),
* call the PubMed tool through an **MCP server** (which performs `esearch.fcgi` + `efetch.fcgi`) and return normalized article summaries.
...
> [!NOTE]
> The overall structure is inspired by [`EchoAgent_2`](../agent_EchoAgent_2/) and built from its GPT-based adaptation [`GPTRespondAgent`](../agent_GPTRespondAgent/) by changing its prompt and moving the PubMed E-utilities call layer into an MCP server to fulfill biomedical literature lookup requests.
...
>
> In the MCP version, this environment variable is read by the **MCP server** (which calls `load_dotenv()`), not by the agent.
...
2. `MyAgent`, a subclass of `SummonerClient`, loads:
   ...
   * **GPT config** from `gpt_config.json` (or `--gpt <path>`), including:
     ...
     * the MCP endpoint URL (for example `mcp_url`),
...
7. The GPT output is expected to be either:
   ...
   * if yes, calls the MCP server tool:

     ```python
     mcp_result = await call_tool_async(
         mcp_url=<from gpt_config.json>,
         tool_name="pubmed_handle_request",
         arguments=tool_args,
     )
     api_result = mcp_result
     performed_call = True
     ```
   * if no, it sets:
...
8. The MCP server tool `pubmed_handle_request` internally:
...
Start the MCP server for PubMed (the agent expects it to be reachable before it can call PubMed tools):

```bash
python agents/agent_MCPPubMedAgent/mcp_server.py
```

Prepare `gpt_config.json` and `id.json` in `agents/agent_MCPPubMedAgent/`.

A **typical `gpt_config.json` for MCPPubMedAgent** (only the important parts):
...
```json
// agents/agent_MCPPubMedAgent/id.json
...
```bash
python agents/agent_MCPPubMedAgent/agent.py
```
...
These scenarios show how `MCPPubMedAgent` consumes input from `InputAgent` and either:
...
# Terminal 3: PubMed MCP server
python agents/agent_MCPPubMedAgent/mcp_server.py

# Terminal 4: MCPPubMedAgent
python agents/agent_MCPPubMedAgent/agent.py
```
...
3. Call the PubMed MCP tool (which performs ESearch + EFetch internally).
...
`MCPPubMedAgent` should interpret this as a PubMed search with a relevance ranking and GPT should output tool args similar to:
...
The agent calls the PubMed MCP tool, which performs ESearch with `sort=relevance` and then EFetches up to 3 PMIDs. You should see a response like:
