...
1. On startup, the agent prepares configuration and storage:
   ...
   * Ensures a minimal messages table in both databases.

   * Sets cadences: `BATCH_INTERVAL_SECONDS = 5.0` (receive flush) and `SEND_POLL_INTERVAL_SECONDS = 1.0` (send polling).

   * Uses **short-lived DB connections** for all operations, so there is nothing to close on shutdown.
...
4. When sending (`@client.send(route="", multi=True)`), the handler polls the **send DB** every `SEND_POLL_INTERVAL_SECONDS` for rows where `state='new'` in FIFO order.
...
5. On shutdown, there are **no persistent database connections to close** (all DB work was short-lived).
