...
make_one() {
  ...
  ffmpeg -y -i "$input" -vf "setpts=$(echo "1/$SPEED" | bc -l)*PTS,fps=${FPS},${SCALE_FILTER},palettegen" "$palette"

  echo "Encoding GIF → $(basename "$output")"
  ffmpeg -y -i "$input" -i "$palette" \
    -filter_complex "setpts=$(echo "1/$SPEED" | bc -l)*PTS,fps=${FPS},${SCALE_FILTER}[x];[x][1:v]paletteuse" \
    "$output"
